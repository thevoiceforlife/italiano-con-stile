...
// ─── FIX 6: shuffle stabile basato su seed della domanda ─────────────────────
...
// ─── FIX 2: Box domanda con bordo colorato ────────────────────────────────────
const CHAR_COLOR = {
...
function QBox({ q }) {
  ...
  return (
    <div style={{
      ...
      border: `2px solid ${c}`, padding: "13px 15px",
    }}>
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 480, margin: "0 auto", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <button onClick={onNext} style={{
          background: isCorrect ? "var(--primary)" : "#CC0000", color: "white",
          padding: "13px 18px", borderRadius: "var(--r)", fontSize: 14, fontWeight: 900,
          border: "none", boxShadow: `0 4px 0 ${isCorrect ? "var(--primary-d)" : "#990000"}`,
  ...
  return (
    <>
      <div style={{ flex: 1, padding: "20px 16px", display: "flex", flexDirection: "column", gap: 14, maxWidth: 480, margin: "0 auto", width: "100%", overflowY: "auto" }}>
        {/* FIX 1+3: CharacterBubble reale con immagine, animazione, audio */}
        <CharacterBubble
          character={q.personaggio}
          text={`${q.intro} / ${q.intro_en}`}
          speakText={q.intro}
          autoSpeak={true}
          feedback={confirmed ? (isCorrect ? "ok" : "err") : null}
        />
          ...
          {shuffled.map(({ o: opt }, si) => {
            ...
            return (
              <button key={si} onClick={() => !confirmed && setSelected(si)} style={{
                background: bg, border: `2px solid ${border}`, color, borderRadius: "var(--r)",
                padding: "12px 14px", textAlign: "left", fontFamily: "inherit", width: "100%",
      ...
      {confirmed
        ? <FeedbackBar isCorrect={isCorrect} feedbackOk={q.feedbackOk} feedbackErr={q.feedbackErr}
            onNext={() => { window.speechSynthesis?.cancel(); onAnswer(isCorrect); }} />
        : <CheckBar disabled={selected === null}
            onCheck={() => { setConfirmed(true); playSound(isCorrect ? "correct" : "wrong"); }} />
      }
    </>
  );
}
          ...
          {[true, false].map(val => {
            ...
            return (
              <button key={String(val)} onClick={() => handleSelect(val)} disabled={confirmed} style={{
                flex: 1, padding: 14, borderRadius: "var(--r)", border: `2px solid ${border}`,
                background: bg, color, fontSize: 14, fontWeight: 900,
                cursor: confirmed ? "default" : "pointer", fontFamily: "inherit", transition: "all 0.15s",
              }}>{val ? "✅ Vero / True" : "❌ Falso / False"}</button>
            );
          })}
        </div>
      </div>
      {confirmed && <FeedbackBar isCorrect={isCorrect} feedbackOk={q.feedbackOk} feedbackErr={q.feedbackErr}
        onNext={() => { window.speechSynthesis?.cancel(); onAnswer(isCorrect); }} />}
    </>
  );
}

// ─── FIX 5: Ascolta e scegli ──────────────────────────────────────────────────
    ...
    <>
      <div style={{ flex: 1, padding: "20px 16px", display: "flex", flexDirection: "column", gap: 14, maxWidth: 480, margin: "0 auto", width: "100%", overflowY: "auto" }}>
        <CharacterBubble
          ...
          speakText={q.intro}
          autoSpeak={true}
          feedback={feedback === "ok" ? "ok" : feedback === "err" ? "err" : null}
        />
        {(q.hint_it || q.hintIt) && (
...
// ─── FIX 4: Abbina coppia — shuffle SEPARATO ─────────────────────────────────
