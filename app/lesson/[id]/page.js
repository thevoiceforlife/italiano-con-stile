...
const CHAR_COLOR = { mario:"#FF9B42", sofia:"#C8A0E8", diego:"#22C55E", gino:"#E5B700", matilde:"#1CB0F6", vittoria:"#E5B700" };
const CHAR_EMOJI = { mario:"☕", sofia:"🎧", diego:"🧢", gino:"🎓", matilde:"💼", vittoria:"🍦" };

// ─── FIX 2: QBox con bordo colorato per distinguerlo dalle risposte ──────────
function QBox({ q }) {
  const c = CHAR_COLOR[q?.personaggio] || "#1CB0F6";
  return (
    <div style={{ background:"var(--card)", borderRadius:"var(--r)", border:`2px solid ${c}`, boxShadow:`inset 0 0 0 1px ${c}11`, padding:"13px 15px" }}>
...
// ─── FIX 3: QBubble — autoSpeak solo una volta, no loop ──────────────────────
function QBubble({ q }) {
  const color = CHAR_COLOR[q.personaggio] || "#FF9B42";
  const spoken = useRef(false);
  useEffect(() => {
    if (!spoken.current && q.intro) {
      spoken.current = true;
  ...
  return (
    <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
      <div style={{ width:42, height:42, borderRadius:"50%", border:`3px solid ${color}`, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center", fontSize:18, background:"var(--card)" }}>
        {CHAR_EMOJI[q.personaggio]||"☕"}
      </div>
      <div style={{ background:"var(--card)", border:`1.5px solid ${color}55`, borderRadius:`0 var(--r) var(--r) var(--r)`, padding:"10px 13px", flex:1 }}>
        <div style={{ fontSize:13, fontWeight:700, color:"var(--text)", lineHeight:1.5 }}>{q.intro}</div>
        {q.intro_en && <div style={{ fontSize:12, color:"var(--text2)", fontStyle:"italic", marginTop:3, lineHeight:1.4 }}>{q.intro_en}</div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ background:isCorrect?"var(--ok-bar)":"var(--err-bar)", borderTop:`2px solid ${isCorrect?"var(--ok-text)":"var(--err-text)"}`, padding:"16px", flexShrink:0 }}>
      <div style={{ maxWidth:480, margin:"0 auto", display:"flex", alignItems:"center", gap:12 }}>
        ...
        <button onClick={onNext} style={{ background:isCorrect?"var(--primary)":"#CC0000", color:"white", padding:"13px 18px", borderRadius:"var(--r)", fontSize:14, fontWeight:900, border:"none", boxShadow:`0 4px 0 ${isCorrect?"var(--primary-d)":"#990000"}`, textTransform:"uppercase", letterSpacing:"0.6px", cursor:"pointer", flexShrink:0, fontFamily:"inherit" }}>
          Avanti →
        </button>
      </div>
    </div>
    ...
    <div style={{ padding:"14px 16px", background:"var(--card)", borderTop:"2px solid var(--border)", flexShrink:0 }}>
      <div style={{ maxWidth:480, margin:"0 auto" }}>
        <button onClick={disabled?undefined:onCheck} disabled={disabled} style={{
          width:"100%", padding:15, borderRadius:"var(--r)", fontSize:15, fontWeight:900, letterSpacing:"0.6px",
          border:"none", textTransform:"uppercase", fontFamily:"inherit",
          background:disabled?"var(--dis-bg)":"var(--primary)", color:disabled?"var(--dis-text)":"white",
          boxShadow:disabled?"none":"0 4px 0 var(--primary-d)", cursor:disabled?"not-allowed":"pointer",
        }}>{label}</button>
      </div>
    </div>
  );
...
// ─── FIX 6: shuffle stabile (seed basato sulla domanda, non Math.random()) ───
function stableShuffleWithCorrect(opzioni, correct, seed) {
  const indexed = opzioni.map((o, i) => ({ o, i }));
  const arr = [...indexed];
  const s = seed || 7;
  ...
  const newCorrect = arr.findIndex(x => x.i === correct);
  return { shuffled: arr, newCorrect };
}

// ─── TIPO: Multipla / Gesti / Falso amico / Scegli registro / Fill blank ──────
function DomandaMultipla({ q, onAnswer }) {
  const seed = (q.domanda?.it||"").split("").reduce((a,c)=>a+c.charCodeAt(0),0);
  const { shuffled, newCorrect } = useRef(stableShuffleWithCorrect(q.opzioni, q.correct, seed)).current;
      ...
      <div style={{ flex:1, padding:"20px 16px", display:"flex", flexDirection:"column", gap:14, maxWidth:480, margin:"0 auto", width:"100%", overflowY:"auto" }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          {[true,false].map(val => {
            ...
            return <button key={String(val)} onClick={()=>handleSelect(val)} disabled={confirmed} style={{ flex:1, padding:14, borderRadius:"var(--r)", border:`2px solid ${border}`, background:bg, color, fontSize:14, fontWeight:900, cursor:confirmed?"default":"pointer", fontFamily:"inherit", transition:"all 0.15s" }}>{val?"✅ Vero / True":"❌ Falso / False"}</button>;
          })}
        </div>
      </div>
...
// ─── FIX 5: Ascolta e scegli — audio stabile, non si resetta ─────────────────
function DomandaAscolta({ q, onAnswer }) {
  ...
  // FIX 5: useEffect vuoto — nessun autoplay, nessun loop
...
function DomandaWordBank({ q, onAnswer }) {
  ...
  const [shuffled] = useState(()=>[...pool].sort(()=>Math.random()-.5));
    ...
    <>
      <div style={{ flex:1, padding:"20px 16px", display:"flex", flexDirection:"column", gap:14, maxWidth:480, margin:"0 auto", width:"100%", overflowY:"auto" }}>
        ...
        <div style={{ fontSize:11, color:"var(--text3)", textTransform:"uppercase", letterSpacing:"0.05em" }}>
          Componi la frase / Build the sentence — tocca per rimuovere / tap to remove:
        </div>
        <div style={{ background:"var(--bg)", border:`2px dashed ${feedback==="ok"?"var(--ok-text)":feedback==="err"?"var(--err-text)":"var(--border)"}`, borderRadius:"var(--r)", padding:"10px 12px", minHeight:50, display:"flex", flexWrap:"wrap", gap:6, alignItems:"center", transition:"border-color 0.2s" }}>
          {target.length===0
            ...
            : target.map((w,i)=>(
                <button key={i} onClick={()=>removeWord(i)} style={{ background:"#C8A0E818", border:"2px solid #C8A0E8", borderRadius:8, padding:"6px 12px", fontSize:13, fontWeight:800, color:"#C8A0E8", cursor:feedback==="ok"?"default":"pointer", fontFamily:"inherit" }}>{w}</button>
              ))
          }
        </div>
...
// ─── FIX 4: Abbina coppia — shuffle SEPARATO, testo bilingue ─────────────────
function DomandaAbbina({ q, onAnswer }) {
  ...
  const shuffIT = useRef([...q.coppie].sort(()=>Math.random()-.5));
  const shuffEN = useRef([...q.coppie].sort(()=>Math.random()-.5));
  ...
  function handleIT(it) {
    if (matched[it]||done) return;
    const newSel = it;
    setSelIT(newSel);
    if (selEN!==null) tryMatch(newSel, selEN);
  }
  function handleEN(en) {
    if (Object.values(matched).includes(en)||done) return;
    const newSel = en;
    setSelEN(newSel);
    if (selIT!==null) tryMatch(selIT, newSel);
  }
...
// ─── FIX 1: VocabIntro — audio solo sulla parola IT ──────────────────────────
function VocabIntro({ lesson, onComplete }) {
  ...
  function handleTap(v) {
    ...
    // FIX 1: pronuncia solo la parola italiana — non la spiegazione
    window.speechSynthesis?.cancel();
    setTimeout(()=>{
          ...
          return (
            <div key={v.id} onClick={()=>handleTap(v)} style={{ background:toccata?"var(--opt-sel-bg)":"var(--card)", border:`2px solid ${toccata?"var(--opt-sel-b)":"var(--border)"}`, borderRadius:"var(--r)", padding:"15px 16px", cursor:"pointer", display:"flex", alignItems:"center", gap:14, boxShadow:toccata?"0 4px 0 var(--opt-sel-b)":"0 4px 0 var(--border)", transition:"background 0.2s, border 0.2s" }}>
              ...
              <div style={{ flex:1 }}>
                ...
                {toccata&&v.mario && <div style={{ fontSize:12, color:"var(--text2)", fontStyle:"italic", marginTop:5, lineHeight:1.4 }}>{v.mario}</div>}
              </div>
...
function QuizFase({ lesson, onComplete }) {
  ...
  return (
    <main style={{ minHeight:"100vh", background:"var(--bg-lesson)", display:"flex", flexDirection:"column" }}>
      ...
      <DomandaRouter key={current} q={q} onAnswer={handleAnswer} />
    </main>
  );
}
